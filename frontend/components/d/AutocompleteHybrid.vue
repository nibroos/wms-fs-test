...
const props = withDefaults(defineProps<AutocompleteType>(), {
  ...
  pageEndCompareProp: 'next_page_url',
...
let loadingSingle = ref<boolean>(false)
...
watch(innerSearch, async (searchValue: string) => {
  ...
  if (isMenuShow.value) {
    ...
    if (props.methodApi === 'post') {
      ...
      response = await useMyFetch().post(props.api, queryObject.value)
    } else {
    ...
    let endProp = property(props.pageEndProp)(resData)
    let endPropCompare = property(props.pageEndCompareProp)(resData)
    if (endProp == endPropCompare) {
      paginationDone.value = true
    } else {
